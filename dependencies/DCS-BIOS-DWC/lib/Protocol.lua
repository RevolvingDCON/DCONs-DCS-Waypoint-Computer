...
function BIOS.protocol.endModule()
  if BIOSdevMode == 1 then
	local function saveDoc()
		...
		local file, err = io.open(lfs.writedir()..[[Scripts\DCS-BIOS-DWC\doc\json\]]..moduleBeingDefined.name..".json", "w")
		local json_string = JSON:encode_pretty(moduleBeingDefined.documentation)
		if file then
			file:write(json_string)
			file:close()
		end
		local file, err = io.open(lfs.writedir()..[[Scripts\DCS-BIOS-DWC\doc\json\]]..moduleBeingDefined.name..".jsonp", "w")
		if file then
